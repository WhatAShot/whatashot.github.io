...
- **<span style="color:SeaGreen">Insights of GLOM</span>**: This work provide a new GLOM method. Previous routing executes clustering by iteractive process, for flexibly aggregating parts into a whole. However, (i) clustering parts according to high-dimensional feature similarity suffers from "curse of dimension"; (ii) an iterative process hinders to build a model deeper; (iii) feature value similarity cannot be used to find semantically cooperative object parts. We separate a clustering process into severel steps as an agglomerative hierachical clustering process, and in each step capsules use its receptors (optimizable templates) to select the "right" object parts. An important constraint we gave is that the receptors of children capsules are recombined to act as receptors of parent capsules, making capsules in different layers execute one agglomerative hierachical clustering, collectively.
</div>
<div markdown="1">
...
- Motivated by how neuron cells work and collaborate, the proposed new capsule structure is equipped with a transmitter and several optimizable receptors, and a capsule works like a neuron cell (i.e., transmitters compress a capsule feauture (an object part) and send the compressed feature to the higher level, and receptors of higher-level capsules semantically match the "right" object parts). The novel skeleton oragnizes all capsules, like a nervous system, to execute an agglomerative hierachical clustering, without iteractive process.
</div>
</div>
